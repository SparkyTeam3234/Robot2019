...
public:
  ...
  int8_t getFPS();
  static void delayMicroseconds(uint16_t delay);
...
template <class LinkType> void TPixy2<LinkType>::delayMicroseconds(uint16_t delay)
{
  frc::Timer dtimer;
  double dly = ((double)delay)/1000000.0; //=0.005
  dtimer.Reset();
  dtimer.Start();
  while (dtimer.Get()<dly) {
    //Spin
  }
}
...
template <class LinkType> int8_t TPixy2<LinkType>::init(uint32_t arg)
{
  ...
  wpi::errs() << "Init:Timeout Loop Start\n";
  frc::Timer dtimer;
  dtimer.Reset();
  for(dtimer.Start(); dtimer.Get()<5.0; )
  {
    wpi::errs() << "Init:Iteration at " << dtimer.Get() << "\n";
